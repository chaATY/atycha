...
这份代码一般用在嵌入式单片机里比较多，比如：设置RTC时钟的时间，从RTC里读取秒单位时间后，需要转换成标准时间显示。

---

# 二、时间转换-秒与标准时间的转换

时间转换(秒与标准时间的转换) (1)函数1: 将秒单位时间转为标准时间。 --RTC实时时钟--->秒为单位--->每秒钟cnt++; 237562867493 -----xxxx年xx月xx日xx时xx分xx秒 星期x。
...
将标准时间转为秒单位的时间。 2018年6月1日19点41分23秒----------xxxxxxx秒 闰年366，平年365。 区分: 每年二月份相差一天. 标准时间基准点: 1970年1月1日0时0分0秒。
